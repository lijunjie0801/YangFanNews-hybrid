...
- (NSString *)encryptString:(NSString *)originalString
{
    ...
    NSString *string = [NewGTMBase64 stringByEncodingData:[NSData dataWithBytes:(const void *)bufferPtr length:(NSUInteger)movedBytes]];
    free(bufferPtr);
    return string;
}
...
{
    NSData *encryptedStringData = [NewGTMBase64 decodeData:[encryptedString dataUsingEncoding:NSUTF8StringEncoding]];
...
- (NSString *)get24BitKey:(NSString *)keyString
{
    ...
    if (keyString.length > 24){
        ...
        keyString = [keyString stringByAppendingFormat:[NSString stringWithFormat:@"%%0%lu",24- keyString.length], 0];
    }
    return keyString;
}
